...
const { decode } = require("./config/startKafka");
...
function CallBack(error, response, body) {
  console.log("[ Kafka Consumer In Node ] GET OAUTH2 RESPONSE STATUS: "+response.statusCode)
...
const run = async () => {
  ...
  await consumerConfig.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      console.log(`[ Kafka Consumer In Node ] New Message! ${message.key}`)
      ...
      console.log(`[ Kafka Consumer In Node ] CPI Integration StatusCode: ${response.statusCode}`);
      console.log(`[ Kafka Consumer In Node ] CPI Integration MessageKey: ${JSON.stringify(message_key)}`);
      return;
      });        
    },
  })
